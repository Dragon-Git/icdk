...
def _verilator_sim_supported() -> bool:
    ...
    (C++20). Most Linux distros work out of the box once a modern ``g++``
    is on PATH, but a few edge cases exist: a Verilator install whose
    ``verilated.mk`` does not carry a ``-std=c++20`` (or coroutine)
    CXX flag will fail to compile the timing runtime; and musl/Alpine
    builds that predate Verilator's PCH fix can trip over the precompiled
    header. Rather than guess from ``/etc/os-release``, we probe that the
    timing header actually compiles with the C++20 standard, which is the
    real requirement. The full build is heavy (a few minutes) so we gate
    the whole test here.
    """
    ...
    if cc is None:
        return False
    # Probe that the coroutine-based timing runtime actually compiles with
    # C++20 on this toolchain (catches Alpine/musl + stale-Verilator PCH
    # breakage, missing libstdc++ coroutine support, etc.) without relying
    # on /etc/os-release.
    root = subprocess.run(
        ["verilator", "--getenv", "VERILATOR_ROOT"],
        capture_output=True, text=True,
    ).stdout.strip()
    timing_hdr = Path(root) / "include" / "verilated_timing.cpp"
    if not timing_hdr.exists():
        return False
    probe = subprocess.run(
        [cc, "-std=c++20", "-fsyntax-only", "-x", "c++",
         "-I", str(Path(root) / "include"),
...
def _run_sim(
    ...
    cmd = (
        ["verilator", "--binary", "--vpi", "--top-module", "tb",
         *LINT_FLAGS,
         # UVM needs Verilator's timing simulator runtime (coroutines
         # in ``verilated_timing.cpp``) which is C++20-only.
         "-CFLAGS", "-std=c++20",
         "-LDFLAGS", "-std=c++20",
         # UVM's DPI layer (uvm_re_*, uvm_dpi_*, vpi_*) lives in
         # uvm-verilator's ``dpi/uvm_dpi.cc``; without it the link step
         # fails with undefined references. ``--vpi`` wires up Verilator's
         # VPI runtime that those symbols call into.
         "--vpi",
         # uvm-bridge-style parallelism: verilate + generate the C++
         # split into N groups so the final ``make`` is -j parallel.
         "-j", str(max(1, os.cpu_count() or 1)),
         "--output-groups", str(max(1, os.cpu_count() or 1)),
         "--Mdir", str(obj_dir),
         *incdirs,
         f"{uvm_src}/uvm_pkg.sv", f"{uvm_src}/dpi/uvm_dpi.cc", if_file,
         f"{SUBMODULE_DIR}/src/pk_syoscb.sv",
         f"{SUBMODULE_DIR}/lib/pk_utils_uvm.sv",
         *pkg_files, tb_file]
    )
    ...
    #
    # ``typical_vseq`` (not ``spi_read_seq``) is the correct top sequence:
    # it is a virtual sequence that routes its inner ``spi_seq_lib`` onto
    # the concrete ``m_spi_sqr`` sequencer. Starting ``spi_read_seq``
    # directly on the virtual sequencer leaves its items unclaimed
    # (no driver behind the virtual sequencer) and the sim hangs forever
    # waiting on the arbitration — exactly the CI timeout this test
    # originally hit.
    r2 = subprocess.run(
        [str(exe), "+UVM_TESTNAME=base_test", "+UVM_TEST_SEQ=typical_vseq"],
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "working coroutine support; set ICDK_SKIP_VERILATOR_SIM=1 to force "
        "a skip even where the toolchain probe would pass)"
    ),
)
